...
- **Runtime as a container** *(v0.16 Pillar A)* — `docker build -f src/Vais.Agents.Runtime.Host/Dockerfile .` → `vais-agents-runtime:0.16.0-preview`. docker-compose recipes in `deploy/compose/`; Helm chart in `deploy/helm/vais-agents-runtime/`. Invoke returns 501 until Pillar B / v0.17 ships manifest-driven instantiation — Create / Get / Delete + OpenAPI + idempotency work today.
- **Everything** — 25 packages + the runtime container; see `artifacts/smoketest/` for what the full library stack looks like.
