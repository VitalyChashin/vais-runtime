...
import { listMcpGatewayEvents } from '../../api/resources'
import { ApiError } from '../../api/client'
import type { McpGatewayEventDto } from '../../api/types'
...
export function McpGatewayEventsTab({ id }: Props) {
  const client = useClient()

  const { data, isLoading, error, refetch, isFetching } = useQuery({
    queryKey: ['mcp-gateway-events', id, client.baseUrl],
    queryFn: () => listMcpGatewayEvents(client, id),
  ...
  return (
    <div className="logs">
      ...
      <div className="logs__scroll">
        ...
        {isUnconfigured && (
          <div className="logs__empty">MCP gateway event store not configured on this runtime.</div>
        )}
        ...
        {!isLoading && !error && data?.map((evt, i) => (
          <McpGatewayEventRow key={`${evt.eventId}-${i}`} evt={evt} />
        ))}
      </div>
    </div>
  )
