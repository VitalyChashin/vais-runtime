...
**Python-plugin agents that call MCP via the plugin's own SDK do not currently emit south trajectories** — the south substrate wires into the per-agent `ToolGatewayMiddleware` chain in the runtime grain, which Python plugins bypass when they hold their own MCP client. Closing this gap (plumbing the south substrate into the runtime's MCP-gateway path, so any agent calling tools through the gateway is observed regardless of host language) is its own future expansion.
