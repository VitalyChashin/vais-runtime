...
/// <summary>
/// In-memory <see cref="ILlmGatewayConfigRegistry"/>. Stores manifests keyed by
...
/// </remarks>
public sealed class InMemoryLlmGatewayConfigRegistry : ILlmGatewayConfigRegistry
{
    private readonly ConcurrentDictionary<(string Id, string Version), LlmGatewayConfigManifest> _manifests = new();
    ...
    public ValueTask<LlmGatewayConfigManifest?> GetAsync(string id, string? version = null, CancellationToken ct = default)
    {
        ...
        LlmGatewayConfigManifest? latest = null;
    ...
    /// <inheritdoc />
    public ValueTask RegisterAsync(LlmGatewayConfigManifest manifest, CancellationToken ct = default)
    {
    ...
    private static bool MatchesLabelPrefix(LlmGatewayConfigManifest manifest, string prefix)
    {
