...
| [SectionedPlugin](SectionedPlugin) | reference Python plugin opting into `POST /v1/sections/build`: fetches the typed `Section[]` view, flattens via `sections_to_openai_request()`, calls back through the gateway-proxied chat-completions endpoint | — (Python + YAML) | ~140 | `OPENAI_API_KEY` (runtime env) | [Wire context sections](../docs/guides/wire-context-sections.md) |
