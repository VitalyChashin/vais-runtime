# 30-second library hello

A library-mode walkthrough: stateful multi-turn chat, first on Semantic Kernel, then on Microsoft Agent Framework, using the same agent class. Source in [`samples/HelloAgent/`](../../samples/HelloAgent/).

> Library mode embeds `StatefulAiAgent` in your own .NET app. If you'd rather declare agents as YAML and let the runtime supervise them, see **[Your first declarative agent](../agent-developer/your-first-declarative-agent.md)** in the Agent developer section.
...
- `gen_ai.*` OpenTelemetry activity + metrics.
...
- **[Choose your stack — MAF or SK](choose-your-stack.md)** — tradeoffs between SK and MAF.
- [Concepts → Execution loop](../concepts/execution-loop.md) — what happens inside `AskAsync` / `StreamAsync`.
- [Concepts → Tools](../concepts/tools.md) — `ITool`, registry, `Tool.FromFunc<TIn,TOut>`.
- [Wire a custom tool](../guides/wire-a-custom-tool.md) — deeper recipe.
