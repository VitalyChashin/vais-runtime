...
/// <list type="bullet">
///   <item><c>AddMemoryStreams("vais.agents.events")</c> — single-process dev/test.</item>
///   <item><c>Vais.Agents.Persistence.Redis.AgenticRedisPersistenceExtensions.UseAgenticRedisStreaming</c> — cross-silo via Redis (depends on <c>Microsoft.Orleans.Streaming.Redis</c>, currently only in alpha — treat as preview-grade).</item>
///   <item><c>AddEventHubStreams("vais.agents.events", ...)</c> — Azure production.</item>
/// </list>
/// </para>
/// <para>
...
public sealed class OrleansAgentEventBus : IAgentEventBus
{
    ...
    /// </summary>
    public const string StreamNamespace = "vais.agents.events";
