...
/// (<c>apiVersion</c> + <c>kind</c> + <c>metadata</c> + <c>spec</c>) — the same wire shape
/// the control-plane HTTP surface speaks. Kept local to the MCP server package to avoid
/// cross-depending on <c>Vais.Agents.Control.Http.Client</c>; both serializers are thin
/// wrappers over the same underlying record and drift is guarded by the envelope
/// round-trip tests on each side.
/// </summary>
internal static class ManifestEnvelopeSerializer
{
