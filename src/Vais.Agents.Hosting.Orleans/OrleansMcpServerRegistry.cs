...
/// <summary>
/// <see cref="IMcpServerRegistry"/> backed by per-id grains with a directory
...
/// </summary>
public sealed class OrleansMcpServerRegistry : IMcpServerRegistry
{
    /// <summary>Primary-key string for the cluster's single <see cref="IMcpServerRegistryDirectoryGrain"/>.</summary>
    public const string DirectoryKey = "vais.agents.mcp-server-registry.directory";
    ...
    public OrleansMcpServerRegistry(IGrainFactory grainFactory)
    {
    ...
    {
        var directory = _grainFactory.GetGrain<IMcpServerRegistryDirectoryGrain>(DirectoryKey);
        ...
        foreach (var id in ids)
        {
            ...
            var grain = _grainFactory.GetGrain<IMcpServerRegistryGrain>(id);
    ...
    /// <inheritdoc />
    public async ValueTask<McpServerManifest?> GetAsync(
    ...
    public async ValueTask RegisterAsync(McpServerManifest manifest, CancellationToken ct = default)
    {
        ...
        var grain = _grainFactory.GetGrain<IMcpServerRegistryGrain>(manifest.Id);
    ...
    public static string SerializeManifest(McpServerManifest manifest) =>
    ...
    public static McpServerManifest DeserializeManifest(string json) =>
        JsonSerializer.Deserialize<McpServerManifest>(json, SerializerOptions)
            ?? throw new InvalidOperationException("McpServerManifest JSON deserialised to null.");
}
