...
using VaisChatRole = Vais.Agents.AgentChatRole;

namespace Vais.Agents.Ai.MicrosoftAgentFramework;
...
/// </para>
/// <para>
/// History is authoritative in the Vais.Agents core; we pass the full turn list
...
/// </para>
/// <para>
/// <b>Tool-call behavior (v0.4.1+):</b> the agent is constructed with
/// <c>UseProvidedChatClientAsIs = true</c> so MAF does NOT add its default
/// <c>FunctionInvokingChatClient</c> wrapping. Both paths surface tool calls to
/// <c>StatefulAiAgent</c>'s dispatcher: the non-streaming path maps
/// <see cref="MeaiFunctionCallContent"/> items on the response message to
/// <see cref="CompletionResponse.ToolCalls"/>; the streaming path accumulates
/// them across MAF's <c>AgentRunResponseUpdate</c> items and emits a terminal
/// <see cref="CompletionUpdate"/> carrying the full list of calls, which
/// <c>StatefulAiAgent.StreamAsync</c>'s outer loop dispatches before re-entering
/// the stream for the next turn.
/// </para>
/// </remarks>
public sealed class MafCompletionProvider : ICompletionProvider, IStreamingCompletionProvider
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        // plus (typically only on the last item) the Usage block. With UseProvidedChatClientAsIs
        // we don't auto-invoke tools; we surface them to StatefulAiAgent's outer streaming loop.
        //
        // Tool-call accumulation: MEAI FunctionCallContent items can appear on any update and
        // are typically fully-formed per callId (MEAI surfaces complete FCCs, not arg-string
        // fragments the way raw OpenAI streaming does). We keep a dict keyed by CallId and
        // keep the latest seen copy (handles both the "one update carries everything" and
        // "late update supersedes" cases). Anonymous/empty call-ids get synthetic keys so
        // providers that omit them still round-trip; real providers always set call ids.
        Dictionary<string, MeaiFunctionCallContent>? functionCalls = null;
        var anonymousCallIndex = 0;
            ...
            .ConfigureAwait(false))
        {
            ...
            {
                if (content is MeaiFunctionCallContent fcc)
                {
                    functionCalls ??= new Dictionary<string, MeaiFunctionCallContent>();
                    var key = string.IsNullOrEmpty(fcc.CallId)
                        ? $"__vais_anon_{anonymousCallIndex++}"
                        : fcc.CallId;
                    functionCalls[key] = fcc;
                }
            }
        ...
        {
            var requests = new List<ToolCallRequest>(functionCalls.Count);
            foreach (var fcc in functionCalls.Values)
            {
    ...
    private ChatClientAgent BuildAgent() =>
        new(
            ...
            {
                Name = _agentName,
                Description = "Vais.Agents MAF-backed agent",
