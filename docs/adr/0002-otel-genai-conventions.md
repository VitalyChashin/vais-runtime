...
- **Supersedes:** no prior convention shipped here — this ADR is the first naming commitment for OTel output.
...
1. **Adopt the GenAI conventions verbatim.** We do not invent `vais.*` or `llm.*` tags where a GenAI name exists.
2. **ActivitySource name: `"Vais.Agents"`.** Exposed as `AgenticDiagnostics.ActivitySourceName`. Consumers call `.AddSource("Vais.Agents")` on their `TracerProviderBuilder`, or use our `AddAgenticInstrumentation()` extension which does it for them.
3. **Meter name: `"Vais.Agents"`.** Same string, different telemetry pillar — consumers reading the constant get both by referencing `AgenticDiagnostics.MeterName`.
...
6. **VAIS-specific extensions use the `vais.*` prefix**, clearly separated from GenAI-native tags. `vais.agent.name`, `vais.user.id`, `vais.tenant.id`, `vais.correlation.id`. The Langfuse enricher translates these onto the `langfuse.*` names Langfuse expects.
...
8. **Zero-cost when no listener.** `ActivitySource.StartActivity(...)` returns null when no listener is registered; `Vais.Agents.Core` tolerates this. Consumers who never reference `Vais.Agents.Observability.OpenTelemetry` pay no allocation.

## Why not ...

| Option | Why rejected |
|---|---|
| Invent our own `vais.llm.*` tag scheme | Consumers already have OTel-aware tooling; a custom scheme means every exporter needs a translation rule. |
| Port legacy `llm.*` tags from pre-GenAI libraries | Those predate the GenAI spec; they ossify a naming that no downstream vendor supports. |
...
- Orleans host (M3) will propagate the `vais.correlation.id` tag from grain context into the ActivitySource.
