...
- **Token counts and model on `python.agent.ask` spans.** `PythonAgentShim.AskAsync` now sets `gen_ai.response.model`, `gen_ai.usage.input_tokens`, and `gen_ai.usage.output_tokens` on the `python.agent.ask` activity from the `AgentInvokeResponse.Usage` list. Langfuse can now display token counts and calculate costs for Python agent invocations alongside C# agents.
- **`OpenAIInstrumentor` auto-wired in `vais-agent-sdk`.** `setup_otel()` in `_otel.py` now calls `OpenAIInstrumentor().instrument()` after configuring the provider (try/except so it's a no-op when the package is absent). Plugin venvs that include `opentelemetry-instrumentation-openai` automatically get per-LLM-call spans nested under `python.agent.ask` with no code changes in the plugin.
- **`opentelemetry-instrumentation-openai` added to demo pipeline venvs.** Both the `langgraph-researcher-live` and `sgr-analyst` Docker build stages now install the package, so every OpenAI call inside those Python agents emits a child span visible in Langfuse.
