...
- **Section pipeline observability (Phase 2, SC-10–SC-16).** Six observability surfaces emit per-turn breakdown data from the section pipeline. Each turn now produces a `SectionTelemetrySnapshot` (Context, TurnIndex, per-section measurements + char/token counts + ratios + outcomes, aggregate budget summary) that fans out to a list of `ISectionTelemetrySink`. Configuration via `StatefulAgentOptions.SectionTelemetrySinks` (default: empty list → zero-cost short-circuit; no sinks means no work).
