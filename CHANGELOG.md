...
- **Ontology Diagnostics Part 3 — Predictive failure-prior induction (branch `feat/ontology-diagnostics-part3-failure-prior-induction`).**
  Closes research §11.3 (induction reading) and §11.6 Phase 3. Adds `FailurePatternInducer` that mines the failure-signal corpus (run-health store + MCP gateway event store via `IFailureSearchService`), groups failures by `(ConceptName, AttributionPath)`, and emits `RecipeProposalKind.FailurePrior` proposals through the existing Plan D review queue.
  - **New types in `Vais.Agents.Abstractions`**: `RecipeProposalKind.FailurePrior = 3` (additive enum member); `FailurePriorBody` record (body of a failure-prior proposal — `AgentName`, `ConceptName`, `AttributionPath`, `ToolName?`, `FailureCount`, `FirstSeen`, `LastSeen`); `FailureAttributionOverlay` record and `FailureOntologyOverlay.Attributions` map (where approved priors land).
  - **`FailurePatternInducer : IRecipeInducer`** in `Vais.Agents.Core` — mines `IFailureSearchService.SearchAsync` (which fans out to `IRunHealthStore` + `IMcpGatewayEventStore`), groups failures, and emits proposals when `Support ≥ MinSupport` (default 5). Confidence is set to `0.0` to signal "fail-rate unavailable" (failure-only corpus; v1 simplification — `SuccessCount` dropped per user decision). Recency-boosted sort (signals within `RecencyWeightHours` = 168 h rank first). Risk level: `Low` (< 10 failures) / `Medium` (≥ 10 failures).
  - **`IFailureOntologyOverlayWriter` + `JsonFailureOntologyOverlayWriter`** in `Vais.Agents.Control.Manifests.Json` — parallel to the north `IOntologyOverlayWriter`; writes approved `FailurePrior` proposals into the `FailureOntologyOverlay` JSON file (`induced.failure-ontology.json` in `VAIS_FAILURE_ONTOLOGY_OVERLAY_PATH`). Idempotent (same concept + path replaces existing entry); atomic temp-file-plus-rename write.
  - **`OverlayPublishingRecipeProposalStoreDecorator`** extended with optional `IFailureOntologyOverlayWriter + failureOverlayPath`; dispatches `FailurePrior` approvals to the failure overlay writer instead of the north overlay writer.
  - **CLI**: `vais recipes propose --source failures [--since <iso8601>]` — runs only the `FailurePatternInducer`; omit `--source` for the existing behavioral inducer. `--source` added to `ProposeRecipesAsync` (client + server + CLI command); server reads the `source` query param and resolves the keyed `IRecipeInducer("failures")` service. `vais recipes list --kind FailurePrior` already works via the existing `--kind` flag.
  - **CompositionRoot**: `FailurePatternInducer` registered as keyed service `"failures"` (requires `VAIS_RUN_HEALTH_STORE_CONNECTION`); `JsonFailureOntologyOverlayWriter` registered when `VAIS_FAILURE_ONTOLOGY_OVERLAY_PATH` is set; decorator wires failure overlay path as `<dir>/induced.failure-ontology.json`.
  - **16 new tests** in `Vais.Agents.Core.Tests` (inducer mining/grouping/sort, single-segment path, Confidence=0.0, overlay merge idempotency, round-trip file write, non-FailurePrior no-op, concurrent write). All 1073 existing tests stay green.
  - **Design deviations from plan** (user-acknowledged or advisory): (a) data source changed from `IInterceptorTeeStore` (plan FP-3) to `IFailureSearchService` — `AttributionPath` is not on `TrajectoryEvent`; only `RunHealthSignalRecord` (and `McpGatewayEvent` via the search service) have it; (b) `SuccessCount` / true `FailRate` dropped (user chose Simplified mode); (c) `IFailureSearchService` caps results at 200 per induction run.
  - **Deferred (not in this PR)**: FP-4 (LLM-assisted name decorator via `LlmAssistedRecipeInducer` — existing decorator already works as a wrap; wiring in CompositionRoot is a follow-up); FP-8 (eval-gated approval behind `VAIS_FAILURE_PRIOR_EVAL_GATE` — eval-suite→agent matching mechanism not yet solid, exit criterion #3 is unmet); FP-11 (live verify transcript against `dev.ps1 start`).
...
- **Container-plugin agents now honour per-agent gateway configuration.** Pre-fix,
  ...
  middleware, silently dropping each calling agent's `LlmGatewayRef`,
  `McpGatewayRef`, `OntologyRef`-bound south cartridge, and Plan C2 delegation
  governance — so a deployer-configured per-agent LLM rate limit / MCP deny-list
  / south validation never fired for plugin agents, and Plan D trajectory
  recording stayed empty for the agent population that matters in real
  deployments. Fix: factored the per-agent chain composition out of
  `AgentManifestTranslator`'s declarative branch into a shared
  `BuildPerAgentChainsAsync` helper applied uniformly to the plugin branch too
  (the load-bearing change — pre-fix `StatefulAgentOptions.ToolGatewayMiddleware`
  for a plugin agent carried only DI-global); exposed
  `IAgentManifestTranslator.ResolvePerAgentChainsAsync` returning a new
  `PerAgentChains(Llm, Tool, Input, Budget)` record; the three container-gateway
  endpoints now call the resolver on every plugin callback (404 on unknown
