...
internal sealed class PostgresMcpEventStore : IMcpEventStore
{
    private readonly string _connectionString;
    private readonly ILogger<PostgresMcpEventStore> _logger;

    private const string Schema = """
        CREATE TABLE IF NOT EXISTS vais_mcp_events (
        ...
        CREATE INDEX IF NOT EXISTS idx_vais_mcp_events_server_id  ON vais_mcp_events(server_id, at DESC);
    ...
    public async Task InitializeAsync(CancellationToken ct = default)
    {
        ...
        _logger.LogDebug("McpEventStore schema applied.");
    }

    public async Task RecordAsync(McpEvent evt, CancellationToken ct = default)
    {
        await using var conn = await OpenAsync(ct).ConfigureAwait(false);
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = """
        ...
        cmd.Parameters.AddWithValue(evt.ServerId);
    ...
    public async Task<IReadOnlyList<McpEvent>> ListAsync(string serverId,
        ...
        CancellationToken ct = default)
    {
        ...
        var conditions = new List<string> { "server_id = $1" };
        var paramIdx = 2;
    ...
    public async Task DeleteOlderThanAsync(DateTimeOffset cutoff, CancellationToken ct = default)
    {
        await using var conn = await OpenAsync(ct).ConfigureAwait(false);
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "DELETE FROM vais_mcp_events WHERE created_at < $1";
        cmd.Parameters.AddWithValue(cutoff);
        var deleted = await cmd.ExecuteNonQueryAsync(ct).ConfigureAwait(false);
        if (deleted > 0)
            _logger.LogInformation("McpEventStore pruned {Count} events older than {Cutoff:u}.", deleted, cutoff);
    }
    ...
    private static McpEvent ReadEvent(NpgsqlDataReader r) =>
