...
public sealed class OrleansAgentSessionProxyTests(OrleansClusterFixture fixture)
{
    ...
    public async Task StatefulAiAgent_Composed_With_Proxy_Session_Writes_Through_Durably()
    {
        ...
        var provider = new global::Vais.Agents.Hosting.Orleans.Tests.FixedReplyProvider("answer");
        var agent = new Vais.Agents.Core.StatefulAiAgent(
            provider,
            new Vais.Agents.Core.StatefulAgentOptions { Session = session });
