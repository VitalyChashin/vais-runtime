...
- **Container-plugin section endpoint contract (Phase 4, SC-20).** `contracts/plugin-container/gateway-internal.md` bumped to v0.25 with the new `POST /v1/sections/build` endpoint. Empty request body; response is the resolver-ordered `Section[]` carrying typed payloads (one per `SectionKind`) plus a `totalChars` aggregate. Packer is **not** run — the plugin picks which sections to use. Same `callToken` + headers regime as `/v1/llm/complete`. A `Plugin-shaped sections` subsection documents the deferred `suppress` filter (manifest or request-side) for plugins that build their own context. Implementation, SDK helper, and reference OpenAI adapter follow in SC-21..SC-24.
