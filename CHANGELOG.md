...
- **Manifest serialization single source of truth — one envelope codec, round-trip guards, generated schema/docs/examples.** Per-kind hand-written manifest (de)serializers are replaced by one generic `EnvelopeCodec` (record ↔ v0.6 `apiVersion`/`kind`/`metadata`/`spec` envelope) in `Vais.Agents.Control.Manifests.Json`, shared by the client (`vais apply`) and server-side serialization; `EnvelopeSerializer` is now thin delegates. Every manifest kind gets a round-trip guard, and JSON Schema + example manifests + reference docs are generated from the records (the single source) under `agentic/contracts/`, each drift-gated.
  - **Round-trip completeness guards.** Nested reflection walkers fail the build when an optional field on a manifest record isn't (de)serialized — Agent, AgentGraph, McpServer, LLM/MCP gateway configs, ContainerPlugin, EvalSuite.
  - **Live data-loss fixes.** The graph apply path silently dropped `ref.runtimeUrl`, `ref.a2aUrl`, node `retryPolicy`, node `handlerRef.assemblyName`, and `stateReducers`; EvalSuite dropped `spec.sampling` + spec-level `assertions` (continuous-mode suites lost config on apply). Both fixed.
  - **`vais get -o yaml/json` is now re-appliable.** Output for the seven envelope kinds is a valid envelope (single item + list), so `apply → get → apply` round-trips; previously `get` emitted a non-re-appliable flat shape. (`-o json` now emits `a2aUrl`/`a2aRemoteAgents`, previously the camelCase-mangled `a2AUrl`/`a2ARemoteAgents`.)
  - **Extension parsers unified.** The JSON loader and the server YAML deserializer now parse the same `ExtensionSpec` fields (container fields, handler `timeoutSeconds`, `secrets`); the YAML deserializer accepts `metadata.id`. (`Spec.Config`, currently unconsumed, is still unparsed.)
  - **Generated contracts** under `agentic/contracts/`: `schemas/<kind>.schema.json` (draft 2020-12, field descriptions from XML doc comments — editor `$schema`), `examples/<kind>.example.yaml`, and `reference/<kind>.md`. Guarded by not-stale tests, structural assertions, example loader re-appliability, and a JSON-Schema correctness gate (codec output validates against the generated schema; test-only `JsonSchema.Net`).
  - **New public API** in `Vais.Agents.Control.Manifests.Json`: `EnvelopeCodec`, `ManifestJsonSchemaGenerator`. Wire contract unchanged (`apiVersion: vais.agents/v1`).
