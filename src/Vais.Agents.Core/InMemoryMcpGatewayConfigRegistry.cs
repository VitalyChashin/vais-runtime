...
/// <summary>
/// In-memory <see cref="IMcpGatewayConfigRegistry"/>. Stores manifests keyed by
...
/// </remarks>
public sealed class InMemoryMcpGatewayConfigRegistry : IMcpGatewayConfigRegistry
{
    private readonly ConcurrentDictionary<(string Id, string Version), McpGatewayConfigManifest> _manifests = new();
    ...
    public ValueTask<McpGatewayConfigManifest?> GetAsync(string id, string? version = null, CancellationToken ct = default)
    {
        ...
        McpGatewayConfigManifest? latest = null;
    ...
    /// <inheritdoc />
    public ValueTask RegisterAsync(McpGatewayConfigManifest manifest, CancellationToken ct = default)
    {
    ...
    private static bool MatchesLabelPrefix(McpGatewayConfigManifest manifest, string prefix)
    {
