...
/// <summary>
/// Validates an MCP server manifest file without registering it.
/// Performs structural checks locally then sends the parsed manifest to
/// <c>POST /v1/mcp-servers/validate</c>. Exits 0 on pass, 1 on any error.
/// </summary>
internal sealed class McpServerValidateCommand : AsyncCommand<McpServerValidateCommand.Settings>
{
    public sealed class Settings : CommandSettings
    {
        [Description("Path to the MCP server manifest file (.yaml / .yml / .json). Use '-' to read from stdin as YAML.")]
    ...
    protected override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<McpServerManifest> manifests;
        try
        {
            ...
            manifests = resources.OfType<ManifestResource.McpServerCase>().Select(r => r.Server).ToList();
        }
        ...
        {
            AnsiConsole.MarkupLine("[red]error[/] no McpServer manifest found in file");
            return ProblemDetailsParser.ExitUsageError;
        }

        if (manifests.Count > 1)
        {
            AnsiConsole.MarkupLine($"[red]error[/] file contains {manifests.Count} mcp-server manifests; mcp-server-validate accepts exactly one");
            return ProblemDetailsParser.ExitUsageError;
        }
        ...
        McpServerValidationResult result;
        try
        {
            result = await client.ValidateMcpServerAsync(manifest, cancellationToken);
        }
