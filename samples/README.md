...
| [SectionedPlugin](SectionedPlugin) | reference Python plugin (**canonical v0.27 path**): `build_sections()` + `complete_from_sections()` — runtime runs flatten + telemetry server-side, full per-section observability symmetry with declarative agents | — (Python + YAML) | ~120 | `OPENAI_API_KEY` (runtime env) | [Wire context sections](../docs/guides/wire-context-sections.md) |
