...
#   - echo plugin image already built:
...
# Plugin directory — runtime scans {dir}/{plugin-name}/plugin.yaml
$pluginDir = Join-Path $env:TEMP "vais-e2e-container-plugins"
$echoPluginDir = Join-Path $pluginDir "echo-plugin"
New-Item -ItemType Directory -Force $echoPluginDir | Out-Null
Copy-Item "$here\plugin.yaml" "$echoPluginDir\plugin.yaml" -Force

Write-Host "[1/6] Build echo plugin images"
docker build -t vais-echo:test  "$e2eRoot\shared\echo-plugin" -q | Out-Null
docker tag vais-echo:test vais-echo:test-v2

Write-Host "[2/6] Publish + start runtime"
...
# Set env vars in the current session so Start-Process inherits the full parent
# environment (PATH, DOTNET_ROOT, etc.) plus these overrides.
$env:VAIS_HOSTING_MODE               = "localhost"
$env:VAIS_PLUGINS_DIRECTORY          = ""
$env:VAIS_CONTAINER_PLUGINS_DIRECTORY = $pluginDir
...
# ── Tests ─────────────────────────────────────────────────────────────────────
Write-Host "[3/5] Wait for plugin to reach Ready state"
...
Write-Host "[4/5] Push new image (hot-reload via runtime API)"
...
Write-Host "[5/5] plugin-status --output json is machine-readable"
