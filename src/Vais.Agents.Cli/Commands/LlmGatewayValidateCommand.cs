...
/// <summary>
/// Validates an LLM gateway config manifest file without registering it.
/// Performs structural checks locally then sends the parsed manifest to
/// <c>POST /v1/llm-gateways/validate</c>. Exits 0 on pass, 1 on any error.
/// </summary>
internal sealed class LlmGatewayValidateCommand : AsyncCommand<LlmGatewayValidateCommand.Settings>
{
    public sealed class Settings : CommandSettings
    {
        [Description("Path to the LLM gateway manifest file (.yaml / .yml / .json). Use '-' to read from stdin as YAML.")]
    ...
    protected override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<LlmGatewayConfigManifest> manifests;
        try
        {
            ...
            manifests = resources.OfType<ManifestResource.LlmGatewayConfigCase>().Select(r => r.Config).ToList();
        }
        ...
        {
            AnsiConsole.MarkupLine("[red]error[/] no LlmGatewayConfig manifest found in file");
            return ProblemDetailsParser.ExitUsageError;
        }

        if (manifests.Count > 1)
        {
            AnsiConsole.MarkupLine($"[red]error[/] file contains {manifests.Count} llm-gateway manifests; llm-gateway-validate accepts exactly one");
            return ProblemDetailsParser.ExitUsageError;
        }
        ...
        LlmGatewayConfigValidationResult result;
        try
        {
            result = await client.ValidateLlmGatewayConfigAsync(manifest, cancellationToken);
        }
